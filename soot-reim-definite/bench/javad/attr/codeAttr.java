...
public class codeAttr extends attrInfo {
  ...
  public codeAttr( String name, int length, 
    ...
    if (codeLen > 0) {
      codeBuf = new byte  /*@RepRep*/  [ codeLen ];  // WEI: move  /*@RepRep*/   to the array
    ...
    if (exceptLen > 0) {
	exceptTab = new   exceptInfo  /*@RepPar*/  [ exceptLen ];

      for (int i = 0; i < exceptLen; i++) {
	  exceptTab[i] = new  /*@RepPar*/  exceptInfo( dStream, constPoolSec );
      }
    }
    ...
    if (attrCount > 0) {
	codeAttributes = new    attrInfo  /*@RepPar*/  [ attrCount ];
