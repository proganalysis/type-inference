...
class classDeclSec extends dataRead implements access_and_modifier_flags {
  ...
  classDeclSec( DataInputStream dStream, constPool constPoolSec ) {
    ...
    if (interfaceCnt > 0) {
      ...
      interfaces = new   /*@ParPar*/  constBase /*@RepPar*/ [ interfaceCnt ];  //WEI: add ParPar
...
class fieldInfo extends dataRead {
  ...
  fieldInfo( DataInputStream dStream, constPool constPoolSec ) {
    ...
    if (attr_cnt > 0) {
	attributes = new    attrInfo  /*@RepPar*/  [ attr_cnt ];
...
class classFieldSec extends dataRead {
  ...
  classFieldSec( DataInputStream dStream, constPool constPoolSec ) {
    ...
    if (field_cnt > 0) {
      classFields =  new   fieldInfo  /*@RepPar*/  [ field_cnt ];
    }

    // initialize the fieldInfo array
    for (int i = 0; i < field_cnt; i++) {
	classFields[i] = new  /*@RepPar*/  fieldInfo( dStream, constPoolSec );
    } // for
...
class classMethodSec extends dataRead {
  ...
  classMethodSec( DataInputStream dStream, 
    ...
    if (methodCnt > 0) {
	classMethods = new    methodInfo  /*@RepPar*/  [ methodCnt ];
    }

    for (int i = 0; i < methodCnt; i++) {
	classMethods[i] = new  /*@RepPar*/  methodInfo( dStream, constPoolSec );
...
class classAttrSec extends dataRead {
  ...
  classAttrSec( DataInputStream dStream, constPool constPoolSec ) {
    ...
    if (numAttr > 0) {
	classAttrTab = new    attrInfo  /*@RepPar*/  [ numAttr ];
...
<p>
  ...
  public classFile(DataInputStream dStream) {
    ...
    // u2 minor_version;
    // u2 major_version;
      header = new  /*@RepOwn*/  classFileHeader( dStream );

    // u2 constant_pool_count;
    // cp_info constant_pool[constant_pool_count-1];
      classConstPool = new  /*@OwnOwn*/  constPool( dStream );
    ...
    // u2 interfaces_count;
    // u2 interfaces[interfaces_count];
      classDecl = new  /*@RepOwn*/  classDeclSec( dStream, classConstPool );

    // u2 fields_count;
    // field_info fields[fields_count];
      classFields = new  /*@RepOwn*/  classFieldSec(dStream, classConstPool);

    className = classDecl.getClassName();

    // u2 methods_count;
    // method_info methods[methods_count];
    classMethods = new  /*@RepOwn*/  classMethodSec(dStream, classConstPool, className );

    // u2 attributes_count;
    // attribute_info attributes[attributes_count];
    classAttrs = new  /*@RepOwn*/  classAttrSec(dStream, classConstPool);
  } // classFile
