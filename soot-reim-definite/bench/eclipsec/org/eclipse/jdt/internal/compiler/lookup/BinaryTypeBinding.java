...
void cachePartsFrom(IBinaryType binaryType, boolean needFieldsAndMethods) {
	...
	if (typeSignature == null) {
		...
		this.superInterfaces = NoSuperInterfaces;
		char /*@OwnOwn*/ [] /*@NoRep*/ [] interfaceNames = binaryType.getInterfaceNames();
		...
		if (wrapper.signature[wrapper.start] == '<') {
			...
			wrapper.start++; // skip '<'
			this.typeVariables = createTypeVariables(wrapper, (/*@OwnPar*/ /*@NoRep*/ BinaryTypeBinding)this);
			wrapper.start++; // skip '>'
		...
		// attempt to find the superclass if it exists in the cache (otherwise - resolve it when requested)
		this.superclass = (ReferenceBinding) environment.getTypeFromTypeSignature(wrapper, NoTypeVariables, (/*@OwnPar*/ /*@NoRep*/ BinaryTypeBinding)this);
		this.tagBits |= 	HasUnresolvedSuperclass;

		this.superInterfaces = NoSuperInterfaces;
		if (!wrapper.atEnd()) {
			// attempt to find each superinterface if it exists in the cache (otherwise - resolve it when requested)
			/*@RepRep*/ ArrayList types = new java.util.ArrayList(2);
			do {
				types.add(environment.getTypeFromTypeSignature(wrapper, NoTypeVariables, (/*@OwnPar*/ /*@NoRep*/ BinaryTypeBinding)this));
			} while (!wrapper.atEnd());
		...
		if (size > 0) {
			...
			for (int i = 0; i < size; i++) {
				...
				TypeBinding type = fieldSignature == null 
					? environment.getTypeFromSignature(binaryField.getTypeName(), 0, -1, false, (/*@OwnPar*/ /*@NoRep*/ BinaryTypeBinding)this) 
					: environment.getTypeFromTypeSignature(new SignatureWrapper(fieldSignature), NoTypeVariables, (/*@OwnPar*/ /*@NoRep*/ BinaryTypeBinding)this);
				FieldBinding field = 
					new FieldBinding(
						binaryField.getName(), 
						type, 
						binaryField.getModifiers() | ExtraCompilerModifiers.AccUnresolved, 
						(/*@OwnPar*/ /*@NoRep*/ BinaryTypeBinding)this, 
...
private MethodBinding createMethod(IBinaryMethod method, long sourceLevel) {
	...
	if (methodSignature == null) { // no generics
		...
		if (size > 0) {
			...
			for (int i = 0; i < numOfParams; i++) {
				...
				if (i >= startIndex)   // skip the synthetic arg if necessary
					parameters[i - startIndex] = environment.getTypeFromSignature(methodDescriptor, index, end, false, (/*@OwnPar*/ /*@NoRep*/ BinaryTypeBinding)this);
				index = end + 1;
			}
		}
		...
		if (!method.isConstructor())
			returnType = environment.getTypeFromSignature(methodDescriptor, index + 1, -1, false, (/*@OwnPar*/ /*@NoRep*/ BinaryTypeBinding)this);   // index is currently pointing at the ')'
	} else {
		...
		if (wrapper.signature[wrapper.start] == '<') {
			...
			// ParameterPart = '<' ParameterSignature(s) '>'
			wrapper.start++; // skip '<'
			typeVars = createTypeVariables(wrapper, (/*@OwnPar*/ /*@NoRep*/ BinaryTypeBinding)this);
			wrapper.start++; // skip '>'
		}
		...
		if (wrapper.signature[wrapper.start] == '(') {
			...
			if (wrapper.signature[wrapper.start] == ')') {
				...
				while (wrapper.signature[wrapper.start] != ')')
					types.add(environment.getTypeFromTypeSignature(wrapper, typeVars, (/*@OwnPar*/ /*@NoRep*/ BinaryTypeBinding)this));
				wrapper.start++; // skip ')'
		...
		if (!method.isConstructor())
			returnType = environment.getTypeFromTypeSignature(wrapper, typeVars, (/*@OwnPar*/ /*@NoRep*/ BinaryTypeBinding)this);
	...
	MethodBinding result = method.isConstructor()
		? new MethodBinding(methodModifiers, parameters, exceptions, (/*@OwnPar*/ /*@NoRep*/ BinaryTypeBinding)this)
		: new MethodBinding(methodModifiers, method.getSelector(), returnType, parameters, exceptions, (/*@OwnPar*/ /*@NoRep*/ BinaryTypeBinding)this);
	if (use15specifics)
...
private TypeVariableBinding[] createTypeVariables(SignatureWrapper wrapper, Binding declaringElement) {
	...
	int rank = 0;
	/*@RepRep*/ ArrayList variables = new ArrayList(1);
	depth = 0;
...
private void initializeTypeVariable(TypeVariableBinding variable, TypeVariableBinding[] existingVariables, SignatureWrapper wrapper) {
	...
	if (wrapper.signature[wrapper.start] == ':') {
		...
		type = (ReferenceBinding) environment.getTypeFromTypeSignature(wrapper, existingVariables, (/*@OwnPar*/ /*@NoRep*/ BinaryTypeBinding)this);
		firstBound = type;
	}
	...
	if (wrapper.signature[wrapper.start] == ':') {
		...
		do {
			wrapper.start++; // skip ':'
			types.add(environment.getTypeFromTypeSignature(wrapper, existingVariables, (/*@OwnPar*/ /*@NoRep*/ BinaryTypeBinding)this));
		} while (wrapper.signature[wrapper.start] == ':');
...
public boolean isEquivalentTo(TypeBinding otherType) {
	...
	switch(otherType.kind()) {
		case Binding.WILDCARD_TYPE :
			return ((WildcardBinding) otherType).boundCheck((/*@OwnPar*/ /*@NoRep*/ BinaryTypeBinding)this);
		case Binding.RAW_TYPE :
			return otherType.erasure() == (/*@OwnPar*/ /*@NoRep*/ BinaryTypeBinding)this;
	}
	return false;
}
