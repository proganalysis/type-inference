...
public class EqualExpression extends BinaryExpression {
	...
	public void checkNullComparison(BlockScope scope, FlowContext flowContext, FlowInfo flowInfo, /*@OwnOwn*/ FlowInfo initsWhenTrue, FlowInfo initsWhenFalse) {
			...
			if (rightCst != Constant.NotAConstant) {
				...
				if (valueRequired) {
					...
					if ((bits & IsReturnedValue) != 0){
						...
						codeStream.generateReturnBytecode((/*@OwnPar*/ /*@NoRep*/ EqualExpression)this);
						// comparison is FALSE
						falseLabel.place();
						codeStream.iconst_1();
					} else {
	...
	public TypeBinding resolveType(BlockScope scope) {
		...
		if (leftType.isBaseType() && rightType.isBaseType()) {
			...
			if ((operatorSignature & 0x0000F) == T_undefined) {
				constant = Constant.NotAConstant;
				scope.problemReporter().invalidOperator((/*@OwnPar*/ /*@NoRep*/ EqualExpression)this, leftType, rightType);
				return null;
			}
		...
		constant = Constant.NotAConstant;
		scope.problemReporter().notCompatibleTypesError((/*@OwnPar*/ /*@NoRep*/ EqualExpression)this, leftType, rightType);
		return null;
	}
	public void traverse(ASTVisitor visitor, BlockScope scope) {
		if (visitor.visit((/*@OwnPar*/ /*@NoRep*/ EqualExpression)this, scope)) {
			left.traverse(visitor, scope);
			right.traverse(visitor, scope);
		}
		visitor.endVisit((/*@OwnPar*/ /*@NoRep*/ EqualExpression)this, scope);
	}
}
