...
public FlowInfo analyseAssignment(BlockScope currentScope, 	FlowContext flowContext, 	FlowInfo flowInfo, Assignment assignment, boolean isCompound) {
	...
	if (isCompound) { // check the variable part is initialized if blank final
		if (binding.isBlankFinal()
			...
			currentScope.problemReporter().uninitializedBlankFinalField(binding, (/*@OwnPar*/ /*@NoRep*/ FieldReference)this);
	...
	if (binding.isFinal()) {
		...
		if (binding.isBlankFinal()
			...
			&& ((receiver.bits & ParenthesizedMASK) == 0) // ((/*@OwnPar*/ /*@NoRep*/ FieldReference)this).x is forbidden
			&& currentScope.allowBlankFinalFieldAssignment(binding)) {
			if (flowInfo.isPotentiallyAssigned(binding)) {
				currentScope.problemReporter().duplicateInitializationOfBlankFinalField(
					binding,
					(/*@OwnPar*/ /*@NoRep*/ FieldReference)this);
			} else {
				flowContext.recordSettingFinal(binding, (/*@OwnPar*/ /*@NoRep*/ FieldReference)this, flowInfo);
			}
			flowInfo.markAsDefinitelyAssigned(binding);
		} else {
			// assigning a final field outside an initializer or constructor or wrong reference
			currentScope.problemReporter().cannotAssignToFinalField(binding, (/*@OwnPar*/ /*@NoRep*/ FieldReference)this);
		}
	}
	return flowInfo;
}
...
public void manageSyntheticAccessIfNecessary(BlockScope currentScope, FlowInfo flowInfo, boolean isReadAccess) {
	...
	if (binding.isPrivate()) {
		if ((currentScope.enclosingSourceType() != this.codegenBinding.declaringClass) 
			...
			currentScope.problemReporter().needToEmulateFieldAccess(this.codegenBinding, (/*@OwnPar*/ /*@NoRep*/ FieldReference)this, isReadAccess);
			return;
		}
...
public TypeBinding resolveType(BlockScope scope) {
	...
	FieldBinding fieldBinding = this.codegenBinding = this.binding = scope.getField(this.receiverType, token, (/*@OwnPar*/ /*@NoRep*/ FieldReference)this);
	if (!fieldBinding.isValidBinding()) {
		constant = Constant.NotAConstant;
		scope.problemReporter().invalidField((/*@OwnPar*/ /*@NoRep*/ FieldReference)this, this.receiverType);
		return null;
	}
	...
	if (isFieldUseDeprecated(fieldBinding, scope, (this.bits & IsStrictlyAssigned) !=0)) {
		scope.problemReporter().deprecatedField(fieldBinding, (/*@OwnPar*/ /*@NoRep*/ FieldReference)this);
	}
	...
	if (fieldBinding.isStatic()) {
		...
		if (!(isImplicitThisRcv
			...
			scope.problemReporter().nonStaticAccessToStaticField((/*@OwnPar*/ /*@NoRep*/ FieldReference)this, fieldBinding);
		}
		if (!isImplicitThisRcv && fieldBinding.declaringClass != receiverType) {
			scope.problemReporter().indirectAccessToStaticField((/*@OwnPar*/ /*@NoRep*/ FieldReference)this, fieldBinding);
		}
	}
...
public void traverse(ASTVisitor visitor, BlockScope scope) {
	if (visitor.visit((/*@OwnPar*/ /*@NoRep*/ FieldReference)this, scope)) {
		receiver.traverse(visitor, scope);
	}
	visitor.endVisit((/*@OwnPar*/ /*@NoRep*/ FieldReference)this, scope);
}
}
