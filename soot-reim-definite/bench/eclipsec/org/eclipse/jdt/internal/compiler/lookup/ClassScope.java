...
public class ClassScope extends Scope {
	...
	private LocalTypeBinding buildLocalType(
		...
		referenceContext.scope = (/*@OwnPar*/ /*@NoRep*/ ClassScope)this;
		referenceContext.staticInitializerScope = new MethodScope((/*@OwnPar*/ /*@NoRep*/ ClassScope)this, referenceContext, true);
		referenceContext.initializerScope = new MethodScope((/*@OwnPar*/ /*@NoRep*/ ClassScope)this, referenceContext, false);

		// build the binding or the local type
		LocalTypeBinding localType = new LocalTypeBinding((/*@OwnPar*/ /*@NoRep*/ ClassScope)this, enclosingType, this.innermostSwitchCase());
		...
		if (referenceContext.memberTypes != null) {
			...
			nextMember : for (int i = 0; i < size; i++) {
				...
				/*@RepNorep*/ ClassScope memberScope = new ClassScope((/*@OwnPar*/ /*@NoRep*/ ClassScope)this, referenceContext.memberTypes[i]);
	...
	private void buildMemberTypes(AccessRestriction accessRestriction) {
		...
		if (referenceContext.memberTypes != null) {
			...
			nextMember : for (int i = 0; i < length; i++) {
				...
				/*@RepNorep*/ ClassScope memberScope = new ClassScope((/*@OwnPar*/ /*@NoRep*/ ClassScope)this, memberContext);
	...
	private void buildMethods() {
		...
		for (int i = 0; i < size; i++) {
			if (i != clinitIndex) {
				/*@RepNorep*/ MethodScope scope = new MethodScope((/*@OwnPar*/ /*@NoRep*/ ClassScope)this, methods[i], false);
	...
	SourceTypeBinding buildType(SourceTypeBinding enclosingType, PackageBinding packageBinding, AccessRestriction accessRestriction) {
		...
		if (enclosingType == null) {
			...
			referenceContext.binding = new SourceTypeBinding(className, packageBinding, (/*@OwnPar*/ /*@NoRep*/ ClassScope)this);
		} else {
			char[][] className = CharOperation.deepCopy(enclosingType.compoundName);
			className[className.length - 1] =
				CharOperation.concat(className[className.length - 1], referenceContext.name, '$');
			referenceContext.binding = new MemberTypeBinding(className, (/*@OwnPar*/ /*@NoRep*/ ClassScope)this, enclosingType);
		}
	...
	private void checkAndSetModifiers() {
		...
		if (isMemberType) {
			...
			Scope scope = (/*@OwnPar*/ /*@NoRep*/ ClassScope)this;
			do {
				switch (scope.kind) {
					case METHOD_SCOPE :
	...
	*/
	...
	public void  checkParameterizedTypeBounds() {
		...
		if (superclass != null) {
			superclass.checkBounds((/*@OwnPar*/ /*@NoRep*/ ClassScope)this);
		}
		TypeReference[] superinterfaces = referenceContext.superInterfaces;
		if (superinterfaces != null) {
			for (int i = 0, length = superinterfaces.length; i < length; i++) {
				superinterfaces[i].checkBounds((/*@OwnPar*/ /*@NoRep*/ ClassScope)this);
			}
		}
		TypeParameter[] typeParameters = referenceContext.typeParameters;
		if (typeParameters != null) {
			for (int i = 0, paramLength = typeParameters.length; i < paramLength; i++) {
				typeParameters[i].checkBounds((/*@OwnPar*/ /*@NoRep*/ ClassScope)this);
			}
		}
	...
	*/
	private boolean connectSuperInterfaces() {
		...
		if (compilerOptions().sourceLevel >= ClassFileConstants.JDK1_5) {
			...
			/*@RepRep*/ Map</*@RepRep*/ Object, /*@RepRep*/ Object> invocations = new HashMap(2);
...
//		}
	...
	private ReferenceBinding findSupertype(TypeReference typeReference) {
		try {
			typeReference.aboutToResolve((/*@OwnPar*/ /*@NoRep*/ ClassScope)this); // allows us to trap completion & selection nodes
			compilationUnitScope().recordQualifiedReference(typeReference.getTypeName());
			this.superTypeReference = typeReference;
			ReferenceBinding superType = (ReferenceBinding) typeReference.resolveSuperType((/*@OwnPar*/ /*@NoRep*/ ClassScope)this);
