...
public class Parser implements  ParserBasicInformation, TerminalTokens, OperatorIds, TypeIds {
	...
	public /*@RepRep*/ JavadocParser javadocParser;
	// used for recovery
...
private static void buildFileForReadableName(
	...
	/*@OwnPar*/ ArrayList entries = new ArrayList();
	...
	for (int i = 0; i < tokens.length; i = i + 3) {
		if("1".equals(tokens[i])) { //$NON-NLS-1$
			...
			/*@NoRep*/ StringBuffer buffer = new StringBuffer();
...
private final static void buildFileForTable(String filename, byte[] bytes) throws java.io.IOException {
	/*@RepNorep*/ FileOutputStream stream = new java.io.FileOutputStream(filename);
	stream.write(bytes);
	stream.close();
	System.out.println(filename + " creation complete"); //$NON-NLS-1$
}
...
public Parser(ProblemReporter problemReporter, boolean optimizeStringLiterals) {
	...
	// javadoc support
	this.javadocParser = new JavadocParser((/*@OwnPar*/ /*@NoRep*/ Parser)this);	
}
...
public RecoveredElement buildInitialRecoveryState(){
	...
	if (this.referenceContext instanceof CompilationUnitDeclaration){
		element = new RecoveredUnit(this.compilationUnit, 0, (/*@OwnPar*/ /*@NoRep*/ Parser)this);
		...
		if (this.referenceContext instanceof AbstractMethodDeclaration){
			element = new RecoveredMethod((AbstractMethodDeclaration) this.referenceContext, null, 0, (/*@OwnPar*/ /*@NoRep*/ Parser)this);
			...
			if (this.referenceContext instanceof TypeDeclaration){
				...
				for (int i = 0; i < type.fields.length; i++){
					...
					if (field != null
						...
						element = new RecoveredInitializer(field, null, 1, (/*@OwnPar*/ /*@NoRep*/ Parser)this);
...
protected void consumeAnnotationTypeDeclaration() {
	...
	typeDecl.checkConstructors((/*@OwnPar*/ /*@NoRep*/ Parser)this);
...
protected void consumeClassDeclaration() {
	...
	//convert constructor that do not have the type's name into methods
	boolean hasConstructor = typeDecl.checkConstructors((/*@OwnPar*/ /*@NoRep*/ Parser)this);
...
protected void consumeEnumDeclaration() {
	...
	//convert constructor that do not have the type's name into methods
	boolean hasConstructor = enumDeclaration.checkConstructors((/*@OwnPar*/ /*@NoRep*/ Parser)this);
	...
	public void getMethodBodies(CompilationUnitDeclaration unit) {
		...
		if (unit.types != null) {
			for (int i = unit.types.length; --i >= 0;)
				unit.types[i].parseMethod((/*@OwnPar*/ /*@NoRep*/ Parser)this, unit);
		}
	...
	ProcessTerminals : for (;;) {
		...
		if (act <= NUM_RULES) {
			...
			try {
				...
				if (!this.hasReportedError){
					this.problemReporter().scannerError((/*@OwnPar*/ /*@NoRep*/ Parser)this, e.getMessage());
...
protected void reportSyntaxErrors(boolean isDietParse, int oldFirstToken) {
	...
	if(isDietParse) {
		...
		int /*@OwnOwn*/ [] /*@NoRep*/ [] intervalToSkip = org.eclipse.jdt.internal.compiler.parser.diagnose.RangeUtil.computeDietRange(types);
		/*@RepNorep*/ DiagnoseParser diagnoseParser = new DiagnoseParser((/*@OwnPar*/ /*@NoRep*/ Parser)this, oldFirstToken, start, end, intervalToSkip[0], intervalToSkip[1], intervalToSkip[2], this.options);
		diagnoseParser.diagnoseParse();
		
		reportSyntaxErrorsForSkippedMethod(types);
		this.scanner.resetTo(start, end);
	} else {
		/*@RepNorep*/ DiagnoseParser diagnoseParser = new DiagnoseParser((/*@OwnPar*/ /*@NoRep*/ Parser)this, oldFirstToken, start, end, this.options);
		diagnoseParser.diagnoseParse();
	}
}
			...
			if(methods != null) {
				for (int j = 0; j < methods.length; j++) {
					...
					if(method.errorInSignature) {
						if(method.isAnnotationMethod()) {
							/*@RepNorep*/ DiagnoseParser diagnoseParser = new DiagnoseParser((/*@OwnPar*/ /*@NoRep*/ Parser)this, TokenNameQUESTION, method.declarationSourceStart, method.declarationSourceEnd, this.options);
							diagnoseParser.diagnoseParse();
						} else {
							/*@RepNorep*/ DiagnoseParser diagnoseParser = new DiagnoseParser((/*@OwnPar*/ /*@NoRep*/ Parser)this, TokenNameDIVIDE, method.declarationSourceStart, method.declarationSourceEnd, this.options);
							diagnoseParser.diagnoseParse();
						}
				...
				for (int j = 0; j < length; j++) {
					if (fields[j] instanceof Initializer) {
						...
						if(initializer.errorInSignature){
							/*@RepNorep*/ DiagnoseParser diagnoseParser = new DiagnoseParser((/*@OwnPar*/ /*@NoRep*/ Parser)this, TokenNameRIGHT_SHIFT, initializer.declarationSourceStart, initializer.declarationSourceEnd, this.options);
							diagnoseParser.diagnoseParse();
						}
					}
				}
