...
public class ParameterizedQualifiedTypeReference extends ArrayQualifiedTypeReference {
	...
	public /*@NoRep*/ TypeReference/*@NoRep*/[][] typeArguments;
	...
	public char [][] getParameterizedTypeName(){
		...
		for (int i = 0; i < length; i++) {
			...
			if (arguments == null) {
				...
				/*@RepRep*/ StringBuffer buffer = new StringBuffer(5);
	...
	private TypeBinding internalResolveType(Scope scope, boolean checkBounds) {
	    ...
	    for (int i = packageBinding == null ? 0 : packageBinding.compoundName.length, max = this.tokens.length; i < max; i++) {
			...
			if (typeIsConsistent && currentType.isStatic() && qualifiedType != null && (qualifiedType.isParameterizedType() || qualifiedType.isGenericType())) {
				scope.problemReporter().staticMemberOfParameterizedType((/*@OwnPar*/ /*@NoRep*/ ParameterizedQualifiedTypeReference)this, scope.environment().createParameterizedType((ReferenceBinding)currentType.erasure(), null, qualifiedType));
		    ...
		    if (args != null) {
				...
				if (isClassScope) {
					...
					if (((ClassScope) scope).detectHierarchyCycle(currentType, (/*@OwnPar*/ /*@NoRep*/ ParameterizedQualifiedTypeReference)this, argTypes))
						return null;
				}

			    TypeVariableBinding[] typeVariables = currentType.typeVariables();
				if (typeVariables == NoTypeVariables) { // check generic
					scope.problemReporter().nonGenericTypeCannotBeParameterized((/*@OwnPar*/ /*@NoRep*/ ParameterizedQualifiedTypeReference)this, currentType, argTypes);
					return null;
				} else if (argLength != typeVariables.length) { // check arity
					scope.problemReporter().incorrectArityForParameterizedType((/*@OwnPar*/ /*@NoRep*/ ParameterizedQualifiedTypeReference)this, currentType, argTypes);
					return null;
				}
				// check parameterizing non-static member type of raw type
				if (typeIsConsistent && !currentType.isStatic() && qualifiedType != null && qualifiedType.isRawType()) {
					scope.problemReporter().rawMemberTypeCannotBeParameterized(
							(/*@OwnPar*/ /*@NoRep*/ ParameterizedQualifiedTypeReference)this, scope.environment().createRawType((ReferenceBinding)currentType.erasure(), qualifiedType), argTypes);
					typeIsConsistent = false;				
				}
				...
				if (isClassScope)
					if (((ClassScope) scope).detectHierarchyCycle(currentType, (/*@OwnPar*/ /*@NoRep*/ ParameterizedQualifiedTypeReference)this, null))
						return null;
				if (currentType.isGenericType()) {
	   			    if (typeIsConsistent && qualifiedType != null && qualifiedType.isParameterizedType()) {
						scope.problemReporter().parameterizedMemberTypeMissingArguments((/*@OwnPar*/ /*@NoRep*/ ParameterizedQualifiedTypeReference)this, scope.environment().createParameterizedType((ReferenceBinding)currentType.erasure(), null, qualifiedType));
						typeIsConsistent = false;
					}
		...
		if (this.dimensions > 0) {
			if (dimensions > 255)
				scope.problemReporter().tooManyDimensions((/*@OwnPar*/ /*@NoRep*/ ParameterizedQualifiedTypeReference)this);
	...
	public void traverse(ASTVisitor visitor, BlockScope scope) {
		if (visitor.visit((/*@OwnPar*/ /*@NoRep*/ ParameterizedQualifiedTypeReference)this, scope)) {
		...
		visitor.endVisit((/*@OwnPar*/ /*@NoRep*/ ParameterizedQualifiedTypeReference)this, scope);
	}
