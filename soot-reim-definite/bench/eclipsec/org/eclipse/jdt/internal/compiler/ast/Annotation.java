	...
	public TypeBinding resolveType(BlockScope scope) {
		...
		nextMember: for (int i = 0, requiredLength = methods.length; i < requiredLength; i++) {
			...
			if (!foundValue && (method.modifiers & ClassFileConstants.AccAnnotationDefault) == 0) {
				scope.problemReporter().missingValueForAnnotationMember((/*@OwnPar*/ /*@NoRep*/ Annotation)this, selector);
			}
		}
		// check unused pairs
		...
		if (this.recipient != null) {
			...
			checkTargetCompatibility: {
				...
				scope.problemReporter().disallowedTargetForAnnotation((/*@OwnPar*/ /*@NoRep*/ Annotation)this);
			}
		}
