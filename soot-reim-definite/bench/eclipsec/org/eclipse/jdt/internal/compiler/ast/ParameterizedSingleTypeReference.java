...
public class ParameterizedSingleTypeReference extends ArrayTypeReference {
	...
	private TypeBinding internalResolveType(Scope scope, ReferenceBinding enclosingType, boolean checkBounds) {
		...
		if (enclosingType == null) {
			...
			if (!this.resolvedType.isValidBinding()) {
				scope.problemReporter().invalidEnclosingType((/*@OwnPar*/ /*@NoRep*/ ParameterizedSingleTypeReference)this, this.resolvedType, enclosingType);
				return null;
			}
			if (isTypeUseDeprecated(this.resolvedType, scope))
				scope.problemReporter().deprecatedType(this.resolvedType, (/*@OwnPar*/ /*@NoRep*/ ParameterizedSingleTypeReference)this);
		}
		...
		if (isClassScope) {
			...
			if (((ClassScope) scope).detectHierarchyCycle(currentType, (/*@OwnPar*/ /*@NoRep*/ ParameterizedSingleTypeReference)this, argTypes))
				return null;
		}

		TypeVariableBinding[] typeVariables = currentType.typeVariables();
		if (typeVariables == NoTypeVariables) { // check generic
			scope.problemReporter().nonGenericTypeCannotBeParameterized((/*@OwnPar*/ /*@NoRep*/ ParameterizedSingleTypeReference)this, currentType, argTypes);
			return null;
		} else if (argLength != typeVariables.length) { // check arity
			scope.problemReporter().incorrectArityForParameterizedType((/*@OwnPar*/ /*@NoRep*/ ParameterizedSingleTypeReference)this, currentType, argTypes);
			return null;
		} else if (!currentType.isStatic() && enclosingType != null && enclosingType.isRawType()){
			scope.problemReporter().rawMemberTypeCannotBeParameterized(
					(/*@OwnPar*/ /*@NoRep*/ ParameterizedSingleTypeReference)this, scope.environment().createRawType((ReferenceBinding)currentType.erasure(), enclosingType), argTypes);
			return null;
		}
		...
		if (this.dimensions > 0) {
			if (dimensions > 255)
				scope.problemReporter().tooManyDimensions((/*@OwnPar*/ /*@NoRep*/ ParameterizedSingleTypeReference)this);
	...
	public void traverse(ASTVisitor visitor, BlockScope scope) {
		if (visitor.visit((/*@OwnPar*/ /*@NoRep*/ ParameterizedSingleTypeReference)this, scope)) {
			for (int i = 0, max = this.typeArguments.length; i < max; i++) {
				this.typeArguments[i].traverse(visitor, scope);
			}
		}
		visitor.endVisit((/*@OwnPar*/ /*@NoRep*/ ParameterizedSingleTypeReference)this, scope);
	}
