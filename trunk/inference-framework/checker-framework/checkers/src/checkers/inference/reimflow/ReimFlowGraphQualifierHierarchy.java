 ...
 * an instance of {@link ReimFlowGraphQualifierHierarchy} instead.
 * @author huangw5
 *
 */
public class ReimFlowGraphQualifierHierarchy extends GraphQualifierHierarchy {
    ...
    public static class InferenceGraphFactory extends GraphFactory {
        ...
        protected QualifierHierarchy createQualifierHierarchy() {
            ...
            return new ReimFlowGraphQualifierHierarchy(this);
        }
    }
    
    private final Set<AnnotationMirror> reimQuals;

	protected ReimFlowGraphQualifierHierarchy(GraphFactory f) {
		super(f);
		reimQuals = AnnotationUtils.createAnnotationSet();
		reimQuals.add(ReimFlowChecker.READONLY);
		reimQuals.add(ReimFlowChecker.POLYREAD);
		reimQuals.add(ReimFlowChecker.MUTABLE);
	}
...
//            System.err.println("WARN: QualifierHierarchy: Empty annotations in lhs: " + lhs + " or rhs: " + rhs + " -- ignored");
        ...
        for (Iterator<AnnotationMirror> it = lhs.iterator(); it.hasNext();) {
        	...
        	if (anno.toString().equals(ReimFlowChecker.READONLY.toString())) {
        		isLhsReadonly = true;
        	}
        }
        ...
        for (AnnotationMirror lhsAnno : lSet) {
            for (AnnotationMirror rhsAnno : rSet) {
				if (isLhsReadonly || rhsAnno.toString().equals(ReimFlowChecker.BOTTOM.toString())) {
