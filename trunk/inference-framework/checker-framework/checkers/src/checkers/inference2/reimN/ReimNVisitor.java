 ...
 *
 */
public class ReimNVisitor extends InferenceTypeVisitor<ReimNChecker> {
	
	private ReimNChecker checker;

	public ReimNVisitor(ReimNChecker checker, CompilationUnitTree root) {
		super(checker, root);
        ...
        do {
			...
			if (leaf.getKind() == Kind.ASSIGNMENT) {
				...
				if (leaf.toString().equals(node.toString())) {
					...
					adaptContextType.addAnnotation(checker.READREAD);
				}
	...
	protected boolean checkMethodInvocability(AnnotatedExecutableType method,
        ...
        if (!checker.isSubtype(treeReceiver, methodReceiver)
        		&& !methodReceiver.hasAnnotation(checker.POLYPOLY)) { // WEI: Ignored polyread
