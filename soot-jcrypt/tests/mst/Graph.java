...
public class Graph {
	...
	/**
	 * Create a graph. This is just another method for creating the graph data
	 * structure.
	 * 
	 * @param numvert
	...
	/**
	 * Add edges to the graph. Edges are added to/from every node in the graph
	 * and a distance is computed for each of them.
	 * 
	 * @param numvert
	...
	/**
	 * Compute the distance between two edges. A random number generator is used
	 * to compute the distance.
	 **/
	private @Sensitive int computeDist(int i, int j, int numvert) {
		int less, gt;
		if (i < j) {
			less = i;
			gt = j;
