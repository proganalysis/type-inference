...
public abstract class ForwardInterProceduralAnalysis<M, N, A> extends InterProceduralAnalysis<M, N, A> {
	...
	public void doAnalysis() {
		...
		while (!worklist.isEmpty()) {
			...
			// If this context has no more nodes to analyze, then take it out of
			// the work-list
			...
			if (node != null) {
				...
				// Handle flow functions depending on whether this is a call
				// statement or not
				if (programRepresentation().isCall(node)) {
					...
					for (M targetMethod : programRepresentation().resolveTargets(currentContext.getMethod(), node)) {
						...
						if (!isLibMethod(targetMethod)) {
							...
							// Check if the called method has a context
							// associated with this entry flow:
							Context<M, N, A> targetContext = getContext(targetMethod, entryValue);
							// If not, then set 'targetContext' to a new context
							// with the given entry flow.
							if (targetContext == null) {
								targetContext = initContext(targetMethod, entryValue);
								if (verbose) {
									System.out.println(
											"[NEW] X" + currentContext + " -> X" + targetContext + " " + targetMethod);
								}
								// System.out.println("ENTRY(X"+targetContext+")
								// = " + entryValue);
							}

							// Store the transition from the calling context and
							// site to the called context.
							contextTransitions.addTransition(callSite, targetContext);

							// Check if the target context has been analysed
							// (surely not if it is just newly made):
							if (targetContext.isAnalysed()) {
								hit = true;
								if (verbose) {
									System.out.println(
											"[HIT] X" + currentContext + " -> X" + targetContext + " " + targetMethod);
								}
								A exitValue = targetContext.getExitValue();
								// System.out.println("EXIT(X"+targetContext+")
								// = " + exitValue);
				...
				// Merge with previous OUT to force monotonicity (harmless if
				// flow functions are monotinic)
				...
				if (callers != null) {
					for (CallSite<M, N, A> callSite : callers) {
						// Extract the calling context and node from the caller
						// site.
						Context<M, N, A> callingContext = callSite.getCallingContext();
						N callNode = callSite.getCallNode();
						// Add the calling unit to the calling context's node
						// work-list.
						callingContext.getWorkList().add(callNode);
						// Ensure that the calling context is on the context
						// work-list.
				...
				if (freeResultsOnTheFly) {
					...
					// If any reachable contexts exist on the work-list, then we
					// cannot free memory
					...
					// If no reachable contexts on the stack, then free memory
					// associated
					// with this context
					if (canFree) {
						for (Context<M, N, A> reachableContext : reachableContexts) {
	...
	/**
	 * Creates a new value context and initialises data flow values for its
	 * nodes.
	 * 
	 * <p>
	 ...
	 * <li>Initialise the IN of entry points with a copy of the given entry
	 * value.</li>
	...
	protected Context<M, N, A> initContext(M method, A entryValue) {
		...
		Context<M, N, A> context = new Context<M, N, A>(method, programRepresentation().getControlFlowGraph(method),
				false);
		...
		// Now, initialise the IN of entry points with a copy of the given entry
		// value.
	...
	/**
	 * Processes the intra-procedural flow function of a statement that does not
	 * contain a method call.
	 * 
	 * @param context
	 ...
	 * start of the call, to handle parameters.
	 * 
	 * @param context
	 *            the value context at the call-site
	 * @param targetMethod
	...
	/**
	 * Processes the inter-procedural flow function for a method call at the end
	 * of the call, to handle return values.
	 * 
	 * @param context
	 *            the value context at the call-site
	 * @param targetMethod
	 ...
	 * @return
	 */
	public abstract boolean isLibMethod(M targetMethod);
