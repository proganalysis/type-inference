...
public class ConstantPropagatorAndFolder extends BodyTransformer
{
	...
	private Map<Value, String> stringHelpers = new HashMap<Value, String>();
        ...
        while (stmtIt.hasNext()) {
			...
			if (stmt instanceof AssignStmt) {
				if (e instanceof InstanceInvokeExpr) {
					...
					if(baseType.toString().equals("java.lang.StringBuilder") && invoke.getName().toString().equals("toString")){
						String constString = stringHelpers.get(base);
						if(constString != null){
							/*System.err*/G.v().out.println("	StringBuilder toString " + constString);
						((AssignStmt)stmt).getRightOpBox().setValue(StringConstant.v(constString));
				...
				if (invoke.isConstructor() && baseType.toString().equals("java.lang.StringBuilder")){
					stringHelpers.put(base, "");
					/*System.err*/G.v().out.println("	stringBuilder Init");
				}
