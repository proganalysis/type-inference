...
public class EnerjChecker extends InferenceChecker {
	...
	public void printResult(PrintWriter out) {
		...
		for (Reference r : references) {
			...
			if (!r.getFileName().equals(fileName)) {
				out.println(fileName + ": " + count);
				fileName = r.getFileName();
				count = 0;
			}
			if (type.toString().contains("@Tainted") && !type.toString().contains("@Safe"))
				count++;
	...
	@Override
	protected void handleInstanceFieldRead(Reference aBase, Reference aField, Reference aLhs, int pos) {
		if (aField.getIdentifier().equals("LIB-Array.length")
				|| aField.getIdentifier().equals("LIB-java.awt.Dimension.width")
				|| aField.getIdentifier().equals("LIB-java.awt.Dimension.height"))
			return;
