 ...
 *
 */
public class ConfidentialityVisitor extends BaseTypeVisitor<ConfidentialityChecker> {

	public ConfidentialityVisitor(ConfidentialityChecker checker, CompilationUnitTree root) {
		super(checker, root);
...
//						&& checker.isDefaultPureMethod((ExecutableElement) overriddenElt) 
        ...
        if ((overrider.getReturnType().getKind() != TypeKind.VOID)
        	...
        	if (!fromLibrary && !checker.isPrimitiveType(overrider.getReturnType())) {
        ...
        for (int i = 0; i < overriderParams.size(); ++i) {
            if (!checker.isSubtype(overriddenParams.get(i), overriderParams.get(i))) {
	        	if (!fromLibrary && !checker.isPrimitiveType(overriddenParams.get(i))) {
...
//		return super.visitAnnotation(node, p);
	 ...
	 *
	 */
    protected class ConfidentialityTypeValidator extends BaseTypeVisitor<ConfidentialityChecker>.TypeValidator {
